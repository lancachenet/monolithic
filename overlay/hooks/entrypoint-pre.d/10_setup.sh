...
# The map generated by 15_generate_maps.sh always includes /data/cache/noslice-hosts.map,
# so the file must exist before nginx starts regardless of NOSLICE_FALLBACK.
# Stored under /data/cache/ so it survives container recreation (always mounted by users).
if [[ ! -f /data/cache/noslice-hosts.map ]]; then
    cp /var/noslice-hosts.map /data/cache/noslice-hosts.map
    chown ${WEBUSER}:${WEBUSER} /data/cache/noslice-hosts.map
fi
...
if [[ "${NOSLICE_FALLBACK}" == "true" ]]; then
    ...
    # Stored under /data/cache/ so it survives container recreation (always mounted by users).
    if [[ ! -f /data/cache/noslice-state.json ]]; then
        echo '{}' > /data/cache/noslice-state.json
        chown ${WEBUSER}:${WEBUSER} /data/cache/noslice-state.json
    fi
